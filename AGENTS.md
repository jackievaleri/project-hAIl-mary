# Agent instructions (project-hAIl-mary)
...
- **CLI**: **Typer** — entrypoint wired in `pyproject.toml` under `[project.scripts]`; implementation lives under `project_hail_mary/`.
...
- **Package code**: `project_hail_mary/` — main library and CLI.
